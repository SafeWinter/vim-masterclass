...
`0` 号寄存器总是会存储 **最近的** 删除内容，而 `1` 到 `9` 号寄存器则存储 **之前的** 删除内容。
...
> **实测：黑洞寄存器和普通寄存器的区别**
>
> 打开 `cutcopypaste.txt` 文件，定位到第 39 行：![](../assets/18-1.png)
>
> 复制第 39 行（`TODO`），然后删除第 41 行（`Fix this line too!`），再查看寄存器：![](../assets/18-2.png)
>
> 此时文件变为：![](../assets/18-3.png)
>
